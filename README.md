...
- Resume: 소개 → 마케팅 경력 → 대표 사례 → 이전 경력 → 역량 순서의 이력 요약. `인쇄 / PDF 저장` 버튼으로 브라우저 인쇄 창을 열고 PDF로 저장할 수 있습니다. 제출용 이력서 1쪽과 포트폴리오 4쪽 PDF를 assets에 제공합니다.
...
홈의 큰 개념 도식을 제거하고 실제 산출물의 요약을 배치했습니다. 각 상세 페이지는 문제·판단과 행동·확인된 결과, 작업 근거, 기존 8개 수행 과정 순서입니다. 퍼널 인터랙션은 광고 분석 상세의 펼침 영역으로 이동했습니다. 실제 캡처가 아닌 원본 요약 자료임을 표시합니다. 홈·이력서에서 제출용 PDF를 다운로드할 수 있습니다. 사이트 내용 변경 시 PDF도 함께 갱신해야 합니다.

## 근거 보강 · 2026-09-24

각 사례의 `decisionEvidence`에 보관 자료 기반 관찰·판단·실행 및 확인 범위를 정리했습니다. 광고는 매칭 유형별 비교, GTM은 축약 코드 발췌, 리서치는 경쟁사 익명 비교를 담습니다. PDF는 `site/assets/resume.pdf`(1쪽), `portfolio.pdf`(4쪽)이며 배포 시 함께 복사됩니다.
